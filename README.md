...
| **Learn** | [Glossary](https://vc-lab.vercel.app/glossary.html) | 251 terms, each with a worked example and a deep dive |
...
A **term of the day** sits at the top, the same one all day, cycling through all 251 before it
repeats. Mark anything **Learned** and the tracker fills — with a streak for consecutive days,
milestone marks at 10/25/50/100/251, and a nudge pointing at whichever section is thinnest.
...
Currently live: **Sprints 1–3** — ten articles, `VI1-1` through `VI1-4`, `VI2-1` through `VI2-3`,
and `VI3-1` through `VI3-3` — **plus `VI7-2`**. All ten articles are summarized in `sprints/`.
Everything else returns a Rebrandly broken-link page: all of Sprint 0, `VI2-4`, `VI3-4`, all of
Sprint 4, all of Sprints 5, 6 and 8, and `VI7-1` / `VI7-3` / `VI7-4`. Sprints 5–8 run inside
Decile Hub and may never have public articles.

`VI4-3` **used to be live and went dark on 2026-07-29.** The 4.3 notes in
`sprints/sprint-4-limited-partners.md` were written from the article while the short link was up.
The govclab target itself — `/2023/08/16/sourcing-and-closing-limited-partners/` — still resolves
directly, so link that rather than `vcl.to/VI4-3`.

**Exception:** `vcl.to/VI7-2` does **not** point to a govclab article — it redirects to a Google
Doc *worksheet* (`docs.google.com/.../copy`) containing the Sprint 2 assignment sets. It is the
one live short link that is not an article; check where each actually lands.
